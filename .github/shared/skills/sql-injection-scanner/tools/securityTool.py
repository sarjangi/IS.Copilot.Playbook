...
# functions directly (scan_file_handler, etc.)

if False:  # Disabled for standalone usage
    scan_file_tool = FunctionTool(
    name="scan_file",
    description="""Scan a single source file for SQL injection vulnerabilities.

Detects unsafe patterns including:
- f-string SQL construction
- String concatenation in queries
- Unparameterized cursor.execute() calls
- %-format SQL interpolation

Supports: .py, .js, .ts, .sql, .cs, .java, .php

Returns findings with line numbers, code snippets, severity, and fix recommendations.""",
    parameters={
        "type": "object",
        "properties": {
            "file_path": {
                "type": "string",
                "description": "Absolute or relative path to the source file to scan"
            }
        },
        "required": ["file_path"]
    },
    ...
    scan_directory_tool = FunctionTool(
    name="scan_directory",
    description="""Scan all source files in a directory for SQL injection vulnerabilities.

Recursively walks the directory (skips venv, node_modules, __pycache__).
...
Supports: .py, .js, .ts, .sql, .cs, .java, .php""",
    ...
    handler=scan_directory_handler
)

    check_parameterized_tool = FunctionTool(
    name="check_parameterized",
    description="""Check whether a SQL code snippet uses safe parameterized queries.

Detects safe patterns (?, :param, %(name)s, ORM) and unsafe patterns (f-strings, concatenation).
Returns a verdict and recommendation if unsafe.""",
    parameters={
        "type": "object",
        "properties": {
            "code_snippet": {
                "type": "string",
                "description": "The SQL or Python/JS code snippet to check"
            }
        },
        "required": ["code_snippet"]
    },
    handler=check_parameterized_handler
)
...
Formats findings by severity with file locations, code snippets, and fix recommendations.

output_format options:
- 'text': Full detailed report (default)
- 'summary': Counts only
- 'json': Raw JSON""",
    parameters={
        "type": "object",
        "properties": {
            "findings": {
                "type": "array",
                "items": {"type": "object"},
                "description": "List of findings from scan_file or scan_directory tools"
            },
            "output_format": {
                "type": "string",
                "enum": ["text", "summary", "json"],
                "description": "Report format: 'text' (default), 'summary', or 'json'"
            }
        },
        "required": ["findings"]
    },
